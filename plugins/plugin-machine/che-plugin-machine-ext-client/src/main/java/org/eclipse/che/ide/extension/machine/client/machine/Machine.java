...
public class Machine {
    ...
    @Inject
    public Machine(MachineLocalizationConstant locale, EntityFactory entityFactory, @Assisted MachineDto descriptor, AppContext appContext) {
    ...
    public String getWsServerExtensionsUrl() {
        final List<Link> links = appContext.getWorkspace().getRuntime().getDevMachine().getLinks();

        for (Link link : links) {
            if (Constants.WSAGENT_REFERENCE.equals(link.getRel())) {
                String url = link.getHref();
                url = url.substring(url.indexOf(':'), url.length());
                boolean isSecureConnection = Window.Location.getProtocol().equals("https:");
                return (isSecureConnection ? "wss" : "ws") + url + Config.getCheExtensionPath() + "/ws";
            }
        }
        return "";
    }
